...
static void Sensor_CalibrateZeroInternal(Sensor_Device_t* pstcDev, uint16_t u16AdcVoltageMv) {
    ...
    SENSOR_DEBUG("Calibration: V_theory=%ld mV, V_meas=%ld mV, ZeroOffset=%ld mV\r\n",
                 (long)s32ZeroTheory, (long)s32ZeroMeas, (long)pstcDev->stcCalibration.s32ZeroOffsetMv);
}
...
#if SENSOR_VOLTAGE_DIVIDER_ENABLE
    SENSOR_DEBUG("Mode: Voltage Divider ENABLED (R1=%ld Ohm, R2=%ld Ohm)\r\n", 
...
DeviceResult_t Sensor_Device_Update(void* handle) {
    ...
    if (!pstcDev->u8Calibrated) {
        ...
        if (u16Voltage > 100 && u16Voltage < 3300) {
            SENSOR_DEBUG("First valid ADC data: %d mV (elapsed=%lu ms), calibrating...\r\n",
