...
/*=============================================================================
 * 调试宏定义
 * 统一在 rtt_manager.h 中统一控制 APP_PARAMS_DBG
 *============================================================================*/
 ...
 * 写方向命令:
 *   bit0 = 1: 激活RS485控制, 未激活时忽略后面指令
 *   bit1 = 1: 停止(关闭 RS485 控制)
 *   bit2 = 1: 急停(取消正转/反转, 关闭 RS485 控制)
 *   bit4 = 1: 正转(即使在运行状态也有效, 与 bit5 互斥)
 *   bit5 = 1: 反转(即使在运行状态也有效, 与 bit4 互斥)
 * 读当前状态:
 *   bit4 = 1: 当前正在正转
 *   bit5 = 1: 当前正在反转
...
#define CTRL_CMD_ABS_SAVE           (0x0040U)   /* bit6: 保存绝对位置到Flash */
...
#define FAULT_BIT_OVERTEMP          (0x0004U)   /* bit2: 过热(硬件支持, 当前未使用) */
...
/*=============================================================================
 * Flash 存储参数默认值宏定义
 * 对应 AppParamRecord_t 结构体字段, 掉电 Flash 保存
 * 注意: 电压阈值单位 0.1V, 电流阈值单位 mA
 *============================================================================*/

/* --- Modbus 协议相关参数 (掉电保存, 暴露给 Modbus) --- */
...
#define PARAM_DEFAULT_CURRENT_DETECT_MS         (100)        /* 过流判定时间 1ms */
...
/* --- 内部参数 (不暴露给 Modbus, 但 Flash 存储, 掉电保留) --- */
...
// #define APP_PARAMS_REALTIME_SIMULATE          /* 开启实时数据模拟(每5秒数据+1) */
#define APP_PARAMS_SIM_DBG                      /* 开启模拟实时数据调试打印 */

/* --- 实时数据源选择 --- */
/* 注释掉将使用模拟实时数据(SimRealtimeData, Keil Debug 可修改);
 * 取消注释将使用实际的 dev_rturn 设备获取实时数据 */
#define APP_PARAMS_USE_DEV_RTURN

/*=============================================================================
 * 核心参数记录结构体 (客户 Modbus 协议定义)
 * 注意: 结构体布局必须与 Flash 中存储的布局一致
 *       head_magic 和 tail_magic 用于 Flash 识别
 *       checksum 用于完整性校验
 *============================================================================*/
#pragma pack(4)
typedef struct {
    ...
    /* 客户 Modbus 协议参数 (保持寄存器, 掉电保存, 暴露给 Modbus) */
    ...
    uint32_t baud_rate;             /* 波特率 (内部参数, 不暴露给Modbus) */
    ...
    uint8_t  overcurrent_trigger_count;  /* 过流触发次数(时间窗口模式下可选使用) */
...
typedef struct {
    ...
    volatile uint16_t fault_set;    /* 故障设置: 写 FAULT_BIT_xxx 可设置对应故障位 */
 ...
 * 全局变量声明
 *============================================================================*/
extern AppParamRecord_t g_AppParam;         /* 应用参数(Flash 存储) */
 ...
 *         须 APP_PARAMS_USE_DEV_RTURN 宏开启时有效
...
/**
 * @brief  实时数据模拟(每5秒数据+1, 用于测试故障位翻转)
 *         须 APP_PARAMS_REALTIME_SIMULATE 宏开启时有效
 */
void RealTime_Simulate(void);

/*=============================================================================
 * 模拟实时数据操作函数 (配合 Keil Debug 窗口修改)
 *============================================================================*/
